/*
    Версия 2 от 2022.10.17 автор ZAN
*/
...
namespace Framework {
    ...
    namespace FileSystem {
        ...
        //
        // Шаблон обеспечивает чтение ini файла
        //
        template <typename T> class TIniBase
        {
        private:
            static_assert(is_string_wstring<T>, "This template requires the type std::string or std::wstring.");

            using TOptionsList = vector<pair<string, string>>;
            vector<pair<string, TOptionsList>> ini; // ini файла
        ...
        protected:
            ...
            void GetGroups(vector<string>& groups)
            {
                ...
                std::transform(begin(ini), end(ini), begin(groups), [] (const auto& value) { return value.first; });
            }
            //
            // Метод возвращает указатель на список параметров группы
            //
            TOptionsList* GetOptions(const string& group, const locale& loc = locale())
            {
                if (auto group_it = find_if(begin(ini), end(ini), [&](const auto& value) { return String::TExtension<string>::Compare(group, value.first, true, loc); }); group_it != end(ini))
                {
            ...
            //
            string* GetValue(const string& key, TOptionsList& options, const locale& loc = locale())
            {
                if (auto option_it = find_if(begin(options), end(options), [&](const auto& value) { return String::TExtension<string>::Compare(value.first, key, true, loc); }); option_it != end(options))
                {
        ...
        public:
            ...
            //
            template<typename Type = T> Type GetValue(T group, T key, const Type& def, bool def_if_empty = false, const locale& loc = locale())
            {
                string _group(String::TConverter<string>::ToString(group, loc));
                string _key(String::TConverter<string>::ToString(key, loc));

                if (TOptionsList* options = GetOptions(_group, loc); options)
                {
                    if (const string* value = GetValue(_key, *options, loc); value)
                    {
            ...
            //
            stdfs::path GetValue(T group, T key, const stdfs::path& def, bool def_if_empty = false, const locale& loc = locale())
            {
            ...
            //
            template<typename Type = T> void SetValue(T group, T key, const Type& value, const locale& loc = locale())
            {
                string _group = String::TConverter<decltype(_group)>::ToString(group, loc);
                string _key = String::TConverter<decltype(_key)>::ToString(key, loc);

                TOptionsList* options = GetOptions(_group, loc);

                if (!options)
                {
                    ini.push_back(pair(_group, TOptionsList()));
                    options = &ini.back().second;
                }

                if (string* _value = GetValue(_key, *options, loc); _value)
                {
                    *_value = String::TConverter<string>::ToString(value, loc);
                }
                else
                {
                    options->push_back(pair(_key, String::TConverter<string>::ToString(value, loc)));
                }
            }
            ...
            //
            void SetValue(T group, T key, const stdfs::path& value, const locale& loc = locale())
            {
                SetValue(group, key, value.string(), loc);
            }
        };

        template <typename T> class TIniReader : public TIniBase<T>
        {
        private:
            using TIniBase<T>::SetValue;
                ...
                if (stdfs::exists(path))
                {
                    if (ifstream ini_file(path.string(), ios::in); ini_file.is_open())
                    {
                        ...
                        for (size_t line_counter(1); !ini_file.eof(); line_counter++)
                        {
                            ...
                            {
                                ini_group = ini_line.erase(0, 1).erase(size(ini_line) - 1, 1);

                                String::TExtension<string>::VoidTrim(ini_group);
                            }
                            else // Найден параметр
                            {
                                ...
                                //
                                if (smatch base_match; regex_match(ini_line, base_match, param_with_comment) && (size(base_match) == 2))
                                {
                                ...
                                if (ini_pair.IsMatched())
                                {
                                    if (!ini_group.empty())
                                    {
                                        ...
                                        T _ini_group = String::TConverter<T>::ToString(ini_group, loc);
                                        T _key = String::TConverter<T>::ToString(ini_pair.first, loc);

                                        TIniBase<T>::SetValue(_ini_group, _key, ini_pair.second, loc);
                                    }
                                    else
                                    {
        ...
        template <typename T> class TIniWriter : public TIniBase<T>
        {
        private:
            using TIniBase<T>::GetValue;
        ...
        public:
            ...
            bool Write(const stdfs::path& path, T& error, const locale& loc = locale())
            {
                if (ofstream ini_file(path.string(), ios::out | ios::trunc); ini_file.is_open())
                {
                    ...
                    TIniBase<T>::GetGroups(groups);

                    for (const auto& group : groups)
                    {
                        ini_file << "[" << group << "]" << endl;

                        if (auto options = TIniBase<T>::GetOptions(group); options)
                        {
                            for (const auto& [_key, _value] : *options)
                            {
                                ini_file << _key << " = " << _value << endl;
                            }
                        }
        ...
        using TStringIniBase = TIniBase<string>;
        using TWStringIniBase = TIniBase<wstring>;

        using TStringIniReader = TIniReader<string>;
        using TWStringIniReader = TIniReader<wstring>;

        using TStringIniWrite = TIniWriter<string>;
        using TWStringIniWrite = TIniWriter<wstring>;
    }
    //
